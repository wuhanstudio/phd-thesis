...
\begin{tabular}{ccacaca}
    \hline
    Queries       & \multicolumn{2}{c}{Google Cloud Vision} & \multicolumn{2}{c}{Imagga}      & \multicolumn{2}{c}{DeepAPI (Ours)} \\
    \hline
...
% \begin{itemize}
%     \item The three most popular classification models (VGG16, ResNet50, and Inceptionv3) for research on black-box attacks provided by Keras Model Zoo \cite{chollet2015keras}.
...
Asfter the initialization process, the square attack then generates square-shaped perturbations to decrease the margin loss $L(C(X), y)$ defined by:
...
% The projected gradient descent (PGD) step is used to update adversarial images $x'$, while the exponentiated gradients (EG) \cite{pmlr-v117-ghai20a} step is used to update gradient priors $\boldsymbol{v}$.
