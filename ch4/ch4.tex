...
% \end{subfigure}
% \hfill
% \caption{Realtime Object Tracking \cite{chiu2020probabilistic}}
% \label{fig.obj_track}
% \end{figure}
...
In summary, we find several research papers that attack 2D SOT (caemera) and 3D MOT ( Lidar). But we do not find attacks against 3D monocular trackers \cite{wu2021track} \cite{hu2022monocular}. Neither do we find research that attacks Transformers for 2D MOT.
...
% \caption{Example System with Integrated DPU \cite{zynq2020}}
% \label{fig.dpu}
% \end{figure}
...
% convolutional layer can be pruned, reducing the number of connections by 9× to 13× without loss of accuracy \cite{han2015learning}. After pruning, we have a simplified neural network for efficient inference.
...
% \caption{Defensive Quantization \cite{lin2019defensive}}
% \label{fig.defensive_quantization}
% \end{figure}

% However, other research proposes that it is the batch normalization layer that makes the model more robust \cite{chai2021separating}. Thus, this is still an active research area that requires further investigation.
