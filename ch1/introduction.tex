...
Following research tries to improve the sample efficiency by using Natural gradient estimation more efficient sampling.
...
On the other hand, local search is another popular black-box optimization. The task of generating adversarial inputs can be approached as a problem of selecting what pixels to attack. Thus, we can use existing
local search methods to search for combinations of pixels to be perturbed. 

\clearpage

Besides gradient estimation and local search, we can also achieve black-box attacks using surrogate model. To train a suggrogate model as the victim using query. but it requires significan amounts of queries and thus is less popular.

Hi

However, we notice some common mistakes in prior research. thereby avoiding mistakes made in prior research that obtained unfair advantages by applying the perturbation before image encoding and pre-processing. We introduce common mistakes and how to accelerate black-box attacks in Chapter \ref{chpt:classification}.
...
\textbf{Human-in-the-Middle: Hardware attack?}
...
\textbf{Adversarial Tracking: Versus traditional methods Kalman filter?}
...
\textbf{Is black-box attack practical?}

Much of the current research in the field of SciML is focused on off-line attacks.

\vspace{0.5cm}

\textbf{How can we benefit from interpretation methods?}
...
\begin{itemize}[label={}]
    \item \textbf{Chapter \ref{chpt:driving}}: we introduce real-time white-box attacks against autonomous driving systems that that generate unnoticeable full-image perturbation.
    \item \textbf{Chapter \ref{chpt:detection}}: we achieve real-time white-box attacks against object detection models. we present real-time white-box attacks against object detection that generate noticeable adversarial patches.
    \item \textbf{Chapter \ref{chpt:tracking}}: we intend to attack object tracking systems,
    \item \textbf{Chapter \ref{chpt:classification}}: we investigate distributed black-box attacks against image classification models. we present adversarial classification, a black-box attack against cloud services of image classification.
    \item \textbf{Chapter \ref{chpt:defence}}: the interpretability of deep learning models and defense methods against adversarial attacks are explored to help robots embrace deep neural networks safely.
    \item \textbf{Chapter \ref{chpt:conclusion}} concludes the thesis. 
\end{itemize}
